...
const [selectComp, setSelectComp] = useState("Student-Home");
    ...
    const checkStudentAccess = async () => {
      ...
      try {
        ...
        const res = await Axios.get(`${API_URL}/SignIn`, {
          ...
          withCredentials: true, // ต้องมีเพื่อให้ cookies ถูกส่ง
        });

        // ตรวจสอบว่า Role เป็น "student" หรือไม่
        if (res.data.role !== "student") {
          alert("Access denied! Students only.");
          navigate("/"); // ไปหน้า login หรือหน้าอื่น
          return;
        }
        ...
        console.error("🔴 Error checking student access:", error.response?.data || error);
        alert("Session expired. Please login again.");
        Cookies.remove("token"); // ลบ token เมื่อเกิดข้อผิดพลาด
  ...
  return(
    ...
    <article className="select-comp-container col-md-10">
      ...
      {selectComp === "Student-ShowTiktok" && <B_ShowTiktok id={student_id} />}
